    ...
    <main className="min-h-screen flex flex-col items-center bg-gray-50 text-gray-900 px-4 py-12">
      ...
      <div className="flex items-center gap-4 mb-12 bg-white p-2 rounded-xl shadow-sm border border-gray-100">
        <button 
          ...
          className={`px-6 py-2 rounded-lg text-sm font-medium transition-colors ${billingCycle === 'monthly' ? 'bg-indigo-600 text-white' : 'text-gray-500 hover:text-black'}`}
        >
          ماهانه
        </button>
        <button 
          onClick={() => setBillingCycle("yearly")}
          className={`px-6 py-2 rounded-lg text-sm font-medium transition-colors ${billingCycle === 'yearly' ? 'bg-indigo-600 text-white' : 'text-gray-500 hover:text-black'}`}
        >
          سالانه (۲ ماه رایگان)
        </button>
      </div>
            ...
            <div className="mb-6">
              {plan.price[billingCycle as keyof typeof plan.price] === 0 ? (
                ...
                <>
                  ...
                  <span className="text-gray-500 mr-1">تومان / ماه</span>
                </>
              )}
            </div>

            <button 
              onClick={() => router.push(plan.price.monthly === 0 ? "/login" : "/dashboard/upgrade")}
      ...
      <p className="mt-12 text-center text-sm text-gray-500 max-w-2xl">
        تمام قیمت‌ها به تومان است. پرداخت‌ها از طریق درگاه امن بانکی زرین‌پال انجام می‌شود. اقدام به ارتقا به معنی پذیرفتن قوانین سایت است.
      </p>
    </main>
  );
}