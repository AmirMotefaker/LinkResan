...
export default function PlanDetailsPage() {
  ...
  const handlePayment = async () => {
    ...
    try {
      const res = await fetch(`${API_URL}/payment/request`, {
        ...
        body: JSON.stringify({ amount: currentPrice, plan_name: plan.name }),
      });
  ...
  // اگر کاربر روی بازگشت کلیک کرد، برود به صفحه اصلی پلن‌ها
  const handleBack = () => {
    router.push("/pricing");
  };
  ...
  if (!plan) {
    return (
      <main className="min-h-screen flex items-center justify-center bg-gray-50">
        <p>پلن پیدا نشد. <button onClick={handleBack} className="text-indigo-600 cursor-pointer">بازگشت</button></p>
      </main>
    );
  }
  ...
  return (
    <main className="min-h-screen flex flex-col items-center bg-gray-50 text-gray-900 px-4 py-12">
      <div className="w-full max-w-3xl">
        <button onClick={handleBack} className="flex items-center gap-2 text-sm text-gray-500 hover:text-indigo-600 mb-8 cursor-pointer">
          <ArrowLeft className="w-4 h-4 rotate-180" />
          بازگشت به پلن‌ها
        </button>

        <div className="bg-white p-8 sm:p-12 rounded-3xl shadow-xl border border-gray-100 flex flex-col items-center text-center">
          ...
          <p className="text-gray-500 mb-8 max-w-md">{plan.desc}</p>

          {plan.monthly > 0 && (
            <div className="flex items-center gap-4 mb-8 bg-gray-50 p-2 rounded-xl shadow-sm border border-gray-100">
              <button 
          ...
          <div className="mb-10">
            {currentPrice === 0 ? (
              <span className="text-5xl font-extrabold text-indigo-600">رایگان</span>
            ) : (
              <>
                <span className="text-5xl font-extrabold text-indigo-600">{toFa(currentPrice.toLocaleString())}</span>
                <span className="text-xl text-gray-500 mr-2">تومان / {billingCycle === 'monthly' ? 'ماه' : 'سال'}</span>
              </>
            )}
          </div>
          ...
          <p className="mt-6 text-xs text-gray-400 max-w-xs mx-auto">پرداخت شما از طریق درگاه امن زرین‌پال انجام می‌شود.</p>
        </div>
      </div>
    </main>
  );
