...
export default function Home() {
  ...
  return (
    <main className="h-screen flex flex-col items-center bg-white text-gray-900 px-4 overflow-hidden">
      ...
      {/* هدر مینیمال (لوگو کلیک شدنی است) */}
      ...
      <section className="flex-grow w-full max-w-2xl flex flex-col items-center justify-center text-center">
        ...
        <form onSubmit={handleShorten} className="w-full flex flex-col items-center">
          <input
            ...
            className="w-full h-14 px-6 text-base bg-gray-50 border-2 border-gray-200 rounded-2xl outline-none focus:border-indigo-500 focus:bg-white transition-all placeholder:text-gray-400 shadow-sm"
            required
          />
          ...
          <button 
            ...
            className="text-xs text-gray-500 hover:text-indigo-600 mt-3 mb-2"
          >
          ...
          <button 
            type="submit" 
            disabled={loading} 
            className="mt-2 h-12 px-10 text-base font-bold bg-indigo-600 hover:bg-indigo-700 disabled:opacity-50 text-white rounded-xl transition-colors flex items-center justify-center gap-2 shadow-lg shadow-indigo-600/20"
          >
      ...
      {/* بخش امکانات */}
      <section className="w-full max-w-5xl grid grid-cols-1 md:grid-cols-3 gap-6 text-center mb-4 flex-shrink-0">
        {features.map((feature, index) => (
          <div key={index} className="flex flex-col items-center p-2">
            <div className="bg-indigo-50 p-2 rounded-xl mb-2 border border-indigo-100">
              <feature.icon className="w-5 h-5 text-indigo-600" />
            </div>
            <h3 className="text-sm font-bold mb-1 tracking-tight">{feature.title}</h3>
            <p className="text-gray-500 text-xs leading-relaxed max-w-xs">{feature.desc}</p>
          </div>
        ))}
      </section>

      {/* فوتر با متن و لینک جدید */}
      <footer className="pb-4 text-gray-400 text-xs text-center flex-shrink-0">
        ساخته شده با ❤️ برای توسعه‌دهندگان ایرانی توسط امیر متفکر -{" "}
        <a href="https://github.com/AmirMotefaker/LinkResan" target="_blank" rel="noopener noreferrer" className="font-bold text-gray-600 hover:text-indigo-600 transition-colors">
          لینک رسان
        </a>
      </footer>
    </main>
