...
const GOOGLE_CLIENT_ID = "653070447275-4jn6a7fjmmocc832bdq2ljo741la970b.apps.googleusercontent.com"; // در مرحله بعد این را پر می‌کنیم
...
function LoginContent() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (isLogin) {
        ...
        router.push("/onboarding"); // هدایت به صفحه انتخاب پلن/مارکتینگ
      } else {
        setIsLogin(true);
  ...
  const handleGoogleLogin = useGoogleLogin({
    ...
    onError: () => {
      setError("عملیات گوگل لغو شد یا ناموفق بود");
    },
  });
    ...
    <main className="min-h-screen flex flex-col items-center justify-center bg-gray-50 px-4">
      <div className="w-full max-w-md bg-white p-8 rounded-2xl shadow-xl border border-gray-100">
        ...
        <button 
          ...
          disabled={loading}
          className="w-full h-12 border-2 border-gray-200 hover:bg-gray-50 text-gray-700 font-medium rounded-xl transition-colors flex items-center justify-center gap-2 mb-4 cursor-pointer disabled:opacity-50"
        >
