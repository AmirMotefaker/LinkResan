...
const features = [
  ...
  { name: "تولید نام با هوش مصنوعی (AI Slug)", linkresan: true, bitly: false, dub: true, short: false, tiny: false, rebrandly: false, blink: false, shorby: false },
...
const columns = [
  ...
  { key: "bitly", label: "Bitly" }, { key: "dub", label: "Dub.co" }, { key: "short", label: "Short.io" },
  { key: "tiny", label: "TinyURL" }, { key: "rebrandly", label: "Rebrandly" }, { key: "blink", label: "BL.INK" }, { key: "shorby", label: "Shorby" },
];
    ...
    <main className="min-h-screen flex flex-col bg-gray-50 dark:bg-gray-900 text-gray-900 dark:text-white px-4 py-12 transition-colors duration-300">
      ...
      <div className="w-full max-w-7xl mx-auto">
        <div className="text-center mb-8">
          <h1 className="text-3xl sm:text-5xl font-extrabold mb-4 flex items-center justify-center gap-2">مقایسه فنی لینک رسان با رقبای جهانی <Sparkles className="w-8 h-8 text-indigo-500" /></h1>
        </div>
        <div className="flex justify-center gap-2 mb-12 bg-white dark:bg-gray-800 p-1.5 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 max-w-md mx-auto">
          <span className="flex-1 py-2.5 rounded-lg text-sm font-bold text-center bg-indigo-50 dark:bg-indigo-900/30 text-indigo-600 dark:text-indigo-400">رقبای جهانی</span>
          <Link href="/compare/iran" className="flex-1 py-2.5 rounded-lg text-sm font-bold text-center text-gray-500 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-gray-700">رقبای ایرانی</Link>
        </div>
        <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-xl border border-gray-100 dark:border-gray-700 overflow-hidden">
          <div className="overflow-x-auto">
            <table className="w-full text-sm text-right border-collapse">
              <thead><tr className="bg-gray-50 dark:bg-gray-900 border-b border-gray-200 dark:border-gray-700"><th className="p-4 font-bold whitespace-nowrap">امکانات</th>{columns.map((col) => (<th key={col.key} className={`p-4 text-center font-bold ${col.highlight ? 'bg-indigo-50 dark:bg-indigo-900/30 text-indigo-600 dark:text-indigo-400' : ''}`}>{col.label}</th>))}</tr></thead>
              <tbody>
                {features.map((row, i) => (<tr key={i} className="border-b border-gray-100 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700/50"><td className="p-4 font-medium">{row.name}</td>{columns.map((col) => (<td key={col.key} className={`p-4 ${col.highlight ? 'bg-indigo-50/50 dark:bg-indigo-900/20' : ''}`}>{renderCell(row[col.key as keyof typeof row])}</td>))}</tr>))}
              </tbody>
            </table>
          </div>
        </div>
